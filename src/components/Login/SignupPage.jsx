...
const SignupPage = () => {
    ...
    const onSubmit = async (formData) => {
        try {
            const token = await signup(formData, profilePic)
            setAuthToken(token)
    ...
    return (
        <section className='aligncenter form_page'>
            <form onSubmit={handleSubmit(onSubmit)} className='authentication_form signup_form'>
                <h2>SignUp Form</h2>
                ...
                <em style={{ alignItems: 'center', display:'flex', justifyContent:'center' }}>
                    Already Registered?{' '}
                    <Link to="/login" state={{ from }} style={{ color: 'red' }}>
                        Login
                    </Link>
                </em>
            </form>
